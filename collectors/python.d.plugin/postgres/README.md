# PostgreSQL monitoring with Khulnasoft

Collects database health and performance metrics.

## Requirements

- `python-psycopg2` package. You have to install it manually and make sure that it is available to the `khulnasoft` user,
...
Edit the `python.d/postgres.conf` configuration file using `edit-config` from the Khulnasoft [config
...
First, navigate to your plugins directory, usually at `/usr/libexec/khulnasoft/plugins.d/`. If that's not the case on your
system, open `khulnasoft.conf` and look for the setting `plugins directory`. Once you're in the plugin's directory, switch
to the `khulnasoft` user.
