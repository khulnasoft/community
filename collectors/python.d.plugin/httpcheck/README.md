# HTTP endpoint monitoring with Khulnasoft
...
Edit
the [`python.d/httpcheck.conf`](https://github.com/khulnasoft/community/blob/master/collectors/python.d.plugin/httpcheck/httpcheck.conf)
configuration file using `edit-config` from the Khulnasoft [config
...
- A system/service/firewall might block Khulnasoft's access if a portscan or
  similar is detected.
