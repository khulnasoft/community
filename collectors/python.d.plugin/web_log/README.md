# Web server log (Apache, NGINX, Squid) monitoring with Khulnasoft
...
Khulnasoft turns this "useless" log file, into a powerful performance and health monitoring tool, capable of detecting, **
...
If Khulnasoft is installed on a system running a web server, it will detect it and it will automatically present a series
...
*[**khulnasoft**](https://my-khulnasoft.io/) charts based on metrics collected by querying the `nginx` API (
i.e. `/stub_status`).*

> [**khulnasoft**](https://my-khulnasoft.io/) supports `apache`, `nginx`, `lighttpd` and `tomcat`. To obtain real-time
...
[**khulnasoft**](https://my-khulnasoft.io/) has a powerful `web_log` plugin, capable of incrementally parsing any number of
web server log files. This plugin is automatically started with [**khulnasoft**](https://my-khulnasoft.io/) and comes,
pre-configured, for finding web server log files on popular distributions. Its configuration is
at `/etc/khulnasoft/python.d/web_log.conf`, like this:
...
> **Important**<br/>Keep in mind [**khulnasoft**](https://my-khulnasoft.io/) runs as user `khulnasoft`. So, make sure
> user `khulnasoft` has access to the logs directory and can read the log file.

## Charts

Once you have all log files configured and [**khulnasoft**](https://my-khulnasoft.io/) restarted, **for each log file** you
will get a section at the [**khulnasoft**](https://my-khulnasoft.io/) dashboard, with the following charts.
...
> configuration [option to disable this chart](https://github.com/khulnasoft/khulnasoft/blob/419cd0a237275e5eeef3f92dcded84e735ee6c58/conf.d/python.d/web_log.conf#L63)
> .
...
time the log is written. Since [**khulnasoft**](https://my-khulnasoft.io/) refreshes this chart every single second, you may
...
> So, [unless you have configured your web server to log the size of requests](https://github.com/khulnasoft/khulnasoft/blob/419cd0a237275e5eeef3f92dcded84e735ee6c58/conf.d/python.d/web_log.conf#L76-L89)
> , the `received` dimension will be always zero.

### Timings

[**khulnasoft**](https://my-khulnasoft.io/) will also render the `minimum`, `average` and `maximum` time the web server needed
...
> So, [unless you have configured your web server to also log timings](https://github.com/khulnasoft/khulnasoft/blob/419cd0a237275e5eeef3f92dcded84e735ee6c58/conf.d/python.d/web_log.conf#L76-L89)
...
[**khulnasoft**](https://my-khulnasoft.io/) can map the URLs found in the log file into categories. You can define these
categories, by providing names and regular expressions in `web_log.conf`.

So, this configuration:

```yaml
nginx_khulnasoft: # name the charts
  ...
  categories:
    ...
    api_other: '^/api/'
    khulnasoft_conf: '^/khulnasoft.conf'
    api_old: '^/(data|datasource|graph|list|all\.json)'
```

Produces the following chart. The `categories` section is matched in the order given. So, pay attention to the order you
give your patterns.
...
**All Time Unique Client IPs** unique ips/s. Counts the unique IPs, since the last [**khulnasoft**](https://my-khulnasoft.io/)
restart.
...
> suggest
> to [disable this chart](https://github.com/khulnasoft/khulnasoft/blob/419cd0a237275e5eeef3f92dcded84e735ee6c58/conf.d/python.d/web_log.conf#L64)
> .

## Alarms

The magic of [**Khulnasoft**](https://my-khulnasoft.io/) is that all metrics are collected per second, and all metrics can be
used or correlated to provide real-time alarms. Out of the box, [**khulnasoft**](https://my-khulnasoft.io/) automatically
attaches the following alarms to all `web_log` charts (i.e. to all log files configured, individually):

| alarm                | description                                                                                                                                                                                                                                                                                            | minimum requests |       warning       |    critical     |
|:---------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|:----------------:|:-------------------:|:---------------:|
...
Khulnasoft alarms are user-configurable. Sample config files can be found under directory `health/health.d` of
the [Khulnasoft GitHub repository](https://github.com/khulnasoft/khulnasoft/).
