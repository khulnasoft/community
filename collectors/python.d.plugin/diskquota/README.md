# Diskquota monitoring with Khulnasoft
...
- `repquota` executable
- Working sudoers configuration that allows the khulnasoft user access to the `repquota` executable
...
Edit the `python.d/diskquota.conf` configuration file using `edit-config` from the Khulnasoft [config
...
```conf
khulnasoft    ALL=(root)    NOPASSWD: /sbin/repquota --output=csv /
Defaults:khulnasoft !log_allowed
```
