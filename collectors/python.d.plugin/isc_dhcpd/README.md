# ISC DHCP monitoring with Khulnasoft

Monitors the leases database to show all active leases for given pools.

## Requirements

- dhcpd leases file MUST BE readable by Khulnasoft
...
Edit the `python.d/isc_dhcpd.conf` configuration file using `edit-config` from the Khulnasoft [config
