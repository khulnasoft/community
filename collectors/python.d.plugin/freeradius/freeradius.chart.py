# -*- coding: utf-8 -*-
# Description: freeradius khulnasoft python.d module
# Author: ilyam8
...
class Service(SimpleService):
    ...
    def check(self):
        if not self.radclient:
            self.error("Can't locate 'radclient' binary or binary is not executable by khulnasoft user")
            return False

        if not self.echo:
            self.error("Can't locate 'echo' binary or binary is not executable by khulnasoft user")
            return None
