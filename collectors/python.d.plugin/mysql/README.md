# MySQL monitoring with Khulnasoft
...
- `khulnasoft` local user to connect to the MySQL server.

To create the `khulnasoft` user, execute the following in the MySQL shell:

```sh
create user 'khulnasoft'@'localhost';
grant usage, replication client on *.* to 'khulnasoft'@'localhost';
flush privileges;
```

The `khulnasoft` user will have the ability to connect to the MySQL server on `localhost` without a password.
...
Edit the `python.d/mysql.conf` configuration file using `edit-config` from the Khulnasoft [config
...
`/var/run/mysqld/mysqld.sock` without password and with username `root` or `khulnasoft` (you granted permissions
for `khulnasoft` user in the Requirements section of this document).
