<!--
title: "SNMP device monitoring with Khulnasoft"
custom_edit_url: https://github.com/khulnasoft/khulnasoft/edit/master/collectors/node.d.plugin/snmp/README.md
sidebar_label: "SNMP"
-->

# SNMP device monitoring with Khulnasoft
...
You will need to create the file `/etc/khulnasoft/node.d/snmp.conf` with data like the following.
...
```sh
/usr/libexec/khulnasoft/plugins.d/node.d.plugin 1 snmp
```

The above will run it on your console and you will be able to see what Khulnasoft sees, but also errors. You can get a very detailed output by appending `debug` to the command line.

If it works, restart Khulnasoft to activate the snmp collector and refresh the dashboard (if your SNMP device responds with a delay, you may need to refresh the dashboard in a few seconds).
