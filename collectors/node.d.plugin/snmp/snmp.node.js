...
// khulnasoft snmp module
// This program will connect to one or more SNMP Agents
//

// example configuration in /etc/khulnasoft/node.d/snmp.conf
/*
{
...
if (khulnasoft.options.DEBUG === true) khulnasoft.debug('loaded', __filename, ' plugin');

khulnasoft.processors.snmp = {
    name: 'snmp',
    ...
    prepare: function (service) {
        var __DEBUG = khulnasoft.options.DEBUG;
        ...
        if (typeof service.snmp_oids === 'undefined' || service.snmp_oids === null || service.snmp_oids.length === 0) {
            ...
            if (__DEBUG === true)
                khulnasoft.debug(service.module.name + ': ' + service.name + ': preparing ' + this.name + ' OIDs');
            ...
            while (chart_keys_len--) {
                ...
                if (__DEBUG === true)
                    khulnasoft.debug(service.module.name + ': ' + service.name + ': indexing ' + this.name + ' chart: ' + c);
                ...
                while (dim_keys_len--) {
                    ...
                    if (__DEBUG === true)
                        khulnasoft.debug(service.module.name + ': ' + service.name + ': indexing ' + this.name + ' chart: ' + c + ', dimension: ' + d + ', OID: ' + oid + ", OID name: " + oidname);
            ...
            if (__DEBUG === true)
                khulnasoft.debug(service.module.name + ': ' + service.name + ': indexed ' + this.name + ' OIDs: ' + khulnasoft.stringify(service.snmp_oids_index));

            // now create the array of OIDs needed by net-snmp
            service.snmp_oids = Object.keys(service.snmp_oids_index);

            if (__DEBUG === true)
                khulnasoft.debug(service.module.name + ': ' + service.name + ': final list of ' + this.name + ' OIDs: ' + khulnasoft.stringify(service.snmp_oids));
    ...
    getdata: function (service, index, ok, failed, callback) {
        ...
        if (__DEBUG === true)
            khulnasoft.debug(service.module.name + ': ' + service.name + ': making ' + slice.length + ' entries request, max is: ' + service.request.max_request_size);

        service.snmp_session.get(slice, function (error, varbinds) {
            if (error) {
                service.error('Received error = ' + khulnasoft.stringify(error) + ' varbinds = ' + khulnasoft.stringify(varbinds));
                ...
                if (__DEBUG === true)
                    khulnasoft.debug(service.module.name + ': ' + service.name + ': got valid ' + service.module.name + ' response: ' + khulnasoft.stringify(varbinds));
                ...
                for (var i = 0; i < varbinds_len; i++) {
                    ...
                    if (net_snmp.isVarbindError(varbinds[i])) {
                        if (__DEBUG === true)
                            khulnasoft.debug(service.module.name + ': ' + service.name + ': failed ' + service.module.name + ' get for OIDs ' + varbinds[i].oid);
                        ...
                        switch (varbinds[i].type) {
                            case net_snmp.ObjectType.OctetString:
                                if (service.snmp_oids_index[varbinds[i].oid].type !== 'title' && service.snmp_oids_index[varbinds[i].oid].type !== 'name') {
                                    ...
                                    if (__DEBUG === true) khulnasoft.debug(service.module.name + ': ' + service.name + ': found ' + service.module.name + ' value of OIDs ' + varbinds[i].oid + ", ObjectType " + net_snmp.ObjectType[varbinds[i].type] + " (" + khulnasoft.stringify(varbinds[i].type) + "), typeof(" + typeof (varbinds[i].value) + "), in JSON: " + khulnasoft.stringify(varbinds[i].value) + ", value = " + value.toString() + " (parsed as float in string)");
                                } else {
                                    // just use the string
                ...
                if (__DEBUG === true)
                    khulnasoft.debug(service.module.name + ': ' + service.name + ': finished ' + service.module.name + ' with ' + ok + ' successful and ' + failed + ' failed values');
            }
    ...
    process: function (service, callback) {
        ...
        if (typeof service.snmp_session === 'undefined' || service.snmp_session === null) {
            ...
            if (snmp_version === net_snmp.Version3) {
                if (__DEBUG === true)
                    khulnasoft.debug(service.module.name + ': ' + service.name + ': opening ' + this.name + ' session on ' + service.request.hostname + ' user ' + service.request.user + ' options ' + khulnasoft.stringify(service.request.options));

                // create the SNMP session
                service.snmp_session = net_snmp.createV3Session(service.request.hostname, service.request.user, service.request.options);
            } else {
                if (__DEBUG === true)
                    khulnasoft.debug(service.module.name + ': ' + service.name + ': opening ' + this.name + ' session on ' + service.request.hostname + ' community ' + service.request.community + ' options ' + khulnasoft.stringify(service.request.options));
            ...
            if (__DEBUG === true)
                khulnasoft.debug(service.module.name + ': ' + service.name + ': got ' + this.name + ' session: ' + khulnasoft.stringify(service.snmp_session));

            // if we later need traps, this is how to do it:
            //service.snmp_session.trap(net_snmp.TrapType.LinkDown, function(error) {
            //  if(error) console.error('trap error: ' + khulnasoft.stringify(error));
            //});
        }
...
var snmp = {
    ...
    // khulnasoft.serviceExecute()
    serviceExecute: function (conf) {
        var __DEBUG = khulnasoft.options.DEBUG;

        if (__DEBUG === true)
            khulnasoft.debug(this.name + ': snmp hostname: ' + conf.hostname + ', update_every: ' + conf.update_every);

        var service = khulnasoft.service({
            name: conf.hostname,
            request: conf,
            update_every: conf.update_every,
            module: this,
            processor: khulnasoft.processors.snmp
        });
        ...
        for (var i = 0; i < chart_keys_len; i++) {
            ...
            if (__DEBUG === true)
                khulnasoft.debug(this.name + ': snmp hostname: ' + conf.hostname + ', examining chart: ' + c);
