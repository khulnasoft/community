<!--
title: "node.d.plugin"
custom_edit_url: https://github.com/khulnasoft/khulnasoft/edit/master/collectors/node.d.plugin/README.md
-->

# node.d.plugin

`node.d.plugin` is a Khulnasoft external plugin. It is an **orchestrator** for data collection modules written in `node.js`.
...
This is a generic checklist for submitting a new Node.js plugin for Khulnasoft.  It is by no means comprehensive.
...
`node.d.plugin` is a Khulnasoft plugin that provides an abstraction layer to allow easy and quick development of data
collectors in node.js. It also manages all its data collectors (placed in `/usr/libexec/khulnasoft/node.d`) using a single
instance of node, thus lowering the memory footprint of data collection.

Of course, there can be independent plugins written in node.js (placed in `/usr/libexec/khulnasoft/plugins`).
...
`node.d.conf`. To edit it on your system, run `/etc/khulnasoft/edit-config node.d.conf`.
...
`JSON` has a very strict formatting. If you get errors from Khulnasoft at `/var/log/khulnasoft/error.log` that a certain
...
To test `node.d.plugin` modules, which are placed in `/usr/libexec/khulnasoft/node.d`, you can run `node.d.plugin` by hand,
like this:
...
/usr/libexec/khulnasoft/plugins.d/node.d.plugin debug 1 X Y Z
```

`node.d.plugin` will run in `debug` mode (lots of debug info), with an update frequency of `1` second, evaluating only
the collector scripts `X` (i.e. `/usr/libexec/khulnasoft/node.d/X.node.js`), `Y` and `Z`.
You can define zero or more modules. If none is defined, `node.d.plugin` will evaluate all modules available.

Keep in mind that if your configs are not in `/etc/khulnasoft`, you should do the following before running `node.d.plugin`:
...
-   a function to process the fetched/manipulate the data fetched. This function will make a number of calls
     to create charts and dimensions and pass the collected values to Khulnasoft.
...
To provide a module called `mymodule`, you have create the file `/usr/libexec/khulnasoft/node.d/mymodule.node.js`, with this structure:

```js
// the processor is needed only
// if you need a custom processor
// other than http
khulnasoft.processors.myprocessor = {
	name: 'myprocessor',
...
var mymodule = {
	processResponse: function(service, data) {
		...
		/* send information to the Khulnasoft server here */
	...
	configure: function(config) {
		...
		if(typeof(config.servers) === 'undefined' || config.servers.length === 0) {
			...
			khulnasoft.service({
				...
				module: this,
				processor: khulnasoft.processors.myprocessor,
...
The config file will contain the contents of `/etc/khulnasoft/node.d/mymodule.conf`.
...
If the config file `/etc/khulnasoft/node.d/mymodule.conf` does not give a `enable_autodetect` or `update_every`, these
will be added by `node.d.plugin`. So you module will always have them.

The configuration file `/etc/khulnasoft/node.d/mymodule.conf` may contain whatever else is needed for `mymodule`.

#### processResponse(data)

`data` may be `null` or whatever the processor specified in the `service` returned.

The `service` object defines a set of functions to allow you send information to the Khulnasoft core about:
...
[snmp collector](https://raw.githubusercontent.com/khulnasoft/khulnasoft/master/collectors/node.d.plugin/snmp/snmp.node.js)_
