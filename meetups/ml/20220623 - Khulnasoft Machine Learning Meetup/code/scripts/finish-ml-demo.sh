...
# run khulnasoft-mlapp via docker
# this is just a little python dash based app we use to protoype sometimes
# https://github.com/andrewm4894/khulnasoft-ml-app
echo "run khulnasoft-mlapp via docker"
sudo docker run -d --network="host" --name=khulnasoft-ml-app-1 \
  -p 29999:29999 \
  --env KHULNASOFTMLAPP_HOSTS=127.0.0.1:19999 \
  --env KHULNASOFTMLAPP_SCRAPE_CHILDREN=yes \
  --restart unless-stopped \
  andrewm4894/khulnasoft-ml-app:latest

# mod group to enable container name discovery
sudo usermod -a -G docker khulnasoft
...
# install gremlin - also useful for doing chaos engineering attacks and seeing how they play out in Khulnasoft.
